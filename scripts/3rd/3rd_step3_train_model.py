...
TRAIN_FILE = Path("data/3rd/tmp/step2_train_racer_level.csv")
MODEL_PATH = Path("data/3rd/tmp/step3_rank_model.pkl")
...
# 特徴量と目的変数の分離
drop_cols = ["rank", "date", "car_no", "race_no", "hit"] if "hit" in df.columns else ["rank", "date", "car_no", "race_no"]
...
# CSVで保存
importance_df.to_csv("data/3rd/tmp/step3_feature_importance.csv", index=False)

# 重要度グラフ（任意）
plt.figure(figsize=(10, 6))
lgb.plot_importance(model, max_num_features=20)
plt.tight_layout()
plt.savefig("data/3rd/tmp/step3_feature_importance.png")
plt.close()