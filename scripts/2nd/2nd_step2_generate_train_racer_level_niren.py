...
RACER_STATS_PATH = Path("data/2nd/tmp/step1_racer_stats.csv")
OUTPUT_PATH = Path("data/2nd/tmp/step2_train_racer_level.csv")
...
columns = [
    ...
    "grade", "venue_id", "prefecture", "race_no", "age", "race_grade"
]
df_all = df_all[columns]
df_all["date"] = pd.to_datetime(df_all["date"])


# One-hot encode race_grade columns if present
if "race_grade" in df_all.columns:
    df_all["race_grade"] = df_all["race_grade"].fillna("Unknown")
    # Add race_grade_score column based on race_grade category
    def map_grade_score(grade):
        if grade == "GP":
            return 5
        elif grade == "G1":
            return 4
    ...
    df_all["race_grade_score"] = df_all["race_grade"].apply(map_grade_score)
    df_all = pd.get_dummies(df_all, columns=["race_grade"], prefix="race_grade")
    # Drop GP column if present
    if "race_grade_GP" in df_all.columns:
        df_all.drop(columns=["race_grade_GP"], inplace=True)
