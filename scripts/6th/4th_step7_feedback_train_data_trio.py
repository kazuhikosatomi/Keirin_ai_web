...
EVAL_DIR = Path("data/4th")
OUTPUT_PATH = Path("data/4th/step7")
...
def main():
    ...
    eval_path = f"data/4th/step6/step6_evaluation_trio_{target_date}.csv"
    result_path = f"data/results/{target_date.year}/results_{target_date}.csv"
    ...
    # car1, car2, car3 を抽出
    eval_df[["car1", "car2", "car3"]] = eval_df["pred_comb"].str.split("-", expand=True).astype(int)

    # レース単位で展開して選手単位に
    hit_rows = []
    for _, row in eval_df.iterrows():
        for car in [row["car1"], row["car2"], row["car3"]]:
            hit_rows.append({
                "date": row["date"],
    ...
    output_path = OUTPUT_PATH / f"step7_train_feedback_only_trio_{target_date}.csv"
