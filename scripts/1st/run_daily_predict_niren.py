...
def run_daily_prediction(today: str, yesterday: str):
    ...
    try:
        subprocess.run(
            ["python", "scripts/1st/1st_step8_generate_final_prediction_niren.py", "--date", today],
            check=True
        )
    except subprocess.CalledProcessError:
    ...
    # Step 9 for today
    print(f"\n🖨️ Step9: 実行中: 1st_step9_generate_pdf_niren.py")
    try:
        subprocess.run(
            ["python", "scripts/1st/1st_step9_generate_pdf_niren.py", "--date", today],
            check=True
        )
    except subprocess.CalledProcessError:
