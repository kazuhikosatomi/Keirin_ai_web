...
def predict_arare(date_str):
    # 入力ファイルパス
    entry_file = f"data/5th/tmp/step4_entry_with_features.csv"
    model_file = f"data/5th/tmp/step3_arare_model.pkl"
    output_file = "data/5th/tmp/step5_arare_predictions.csv"
    ...
    if "rank" not in df.columns:
        print("⚠️ 'rank' カラムが存在しません。step4で付与されていればそのまま保存されます。")
