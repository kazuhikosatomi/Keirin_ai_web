...
def main():
    ...
    with open(filename, "w", newline="", encoding="utf-8") as f:
        ...
        # 開催情報のフィールドを最後に追加
        for field in ["race_day_label", "race_grade", "event_name", "race_class"]:
            if field not in renamed_fieldnames:
                renamed_fieldnames.append(field)
        ...
        for row in all_entries:
            ...
            # 開催情報（race_day_label, grade, event_name, race_class）を各エントリに追加
            for field in ["race_day_label", "grade", "event_name", "race_class"]:
                renamed_key = "race_grade" if field == "grade" else field
                if field in result:
                    renamed_row[renamed_key] = result[field]
                else:
                    renamed_row[renamed_key] = ""
